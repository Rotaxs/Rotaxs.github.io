...
在编程中，我们更常用的是算术右移，因为算术右移首先保证负数除以 2 的幂次后不会得到一个整数，其实（比较隐蔽的）是，负数通过算术右移后也可以得到原数除以 2 后的正确结果，比如
...
欲了解用 C 语言模拟`bis`和`bic`的底层实现可参考：[位运算的实用技巧 - 位向量的基本操作](/computer/其他/位运算的实用技巧/#位向量的基本操作)

...
关于格式化字符，可以参考笔者的这篇文章 {% post_link  "C 语言中的格式化输出和转义字符"  "C语言中的格式化输出和转义字符" %}
...
这里 `num` 的值与字符 `p` 没有任何关系，它表示一个垃圾值，`a = 0` 表示 `num` 没能被分配值
...
可以看到，`scanf()` 忽略了前导空白字符和前导 0，而直接读取了 10，`a = 1` 表示成功读取一个变量
...
**EOF（End Of File）**：当 `scanf()`，`getchar()` 等读取数据时发现没东西可读时，会返回 `EOF`，这是一个定义在 `stdio.h` 中的宏，通常为 -1。在 Windows 中，我们可以通过 `Ctrl + Z`（Linux/MaxOS 是 `Ctrl + D`）模拟发送一个 `EOF`

{% endnote %}

对缓冲机制感兴趣的读者可移步至 [缓冲机制](#buffer)
...
需要注意的是，有的读者可能见过 `gets()` 函数，但这个函数不安全，且在现代 C 标准中已经被废弃了
